...
export const eventRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    schema: {
      querystring: {
        ...
        properties: {
          ...
          startAtFrom: { type: 'string', format: 'date-time' },
          startAtTo: { type: 'string', format: 'date-time' },
          visibility: { type: 'string', enum: ['public', 'unlisted', 'invite_only', 'approval_only'] },
  ...
  fastify.post('/', {
        ...
        properties: {
          ...
          venue: {
            ...
            properties: {
              ...
              visibility: { type: 'string', enum: ['public', 'fuzzy', 'private_until_rsvp'] },
            },
          },
          categories: { type: 'array', items: { type: 'string' } },
          tags: { type: 'array', items: { type: 'string' } },
    ...
    try {
      ...
      if (ticketTypes.length === 0) {
        await fastify.prisma.ticketType.create({
          data: {
            eventId: event.id,
    ...
    try {
      ...
      if (request.headers.authorization) {
        try {
          ...
          userTicket = await fastify.prisma.ticket.findUnique({
            where: {
              eventId_userId: {
                eventId: id,
                userId,
  ...
  fastify.patch('/:id', {
    ...
    try {
      ...
      if (updateData.venue) {
        ...
        if (venueId) {
          await fastify.prisma.venue.update({
            where: { id: venueId },
            data: {
      ...
      // Update the event
      const updatedEvent = await fastify.prisma.event.update({
        where: { id },
        data: {
          ...eventUpdateData,
          venueId,
    ...
    schema: {
      ...
      body: {
        ...
        properties: {
          ...
          plusOnes: { type: 'integer', minimum: 0, maximum: 10 },
        },
      },
    },
  }, async (request, reply) => {
    try {
      ...
      if (ticketTypeId) {
        ...
        if (ticketType.capacity && status === 'going') {
          const ticketCount = await fastify.prisma.ticket.count({
            where: {
              eventId,
              ticketTypeId,
              status: 'going',
      ...
      const existingTicket = await fastify.prisma.ticket.findUnique({
        where: {
          eventId_userId: {
            eventId,
            userId,
      ...
      if (status === 'waitlist') {
        ...
        const existingWaitlist = await fastify.prisma.waitlist.findUnique({
          where: {
            eventId_userId: {
              eventId,
              userId,
        ...
        if (!existingWaitlist) {
          ...
          const highestPriority = await fastify.prisma.waitlist.findFirst({
            where: { eventId },
            orderBy: { priority: 'desc' },
          });
          
          // Create waitlist entry with incremented priority
          await fastify.prisma.waitlist.create({
            data: {
              eventId,
              userId,
        ...
        await fastify.prisma.waitlist.delete({
          where: {
            eventId_userId: {
              eventId,
              userId,
  ...
  fastify.get('/:id/attendees', {
    ...
    try {
      ...
      // Get total count
      const total = await fastify.prisma.ticket.count({
        where: {
          eventId,
          ...statusFilter,
        },
